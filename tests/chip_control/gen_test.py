...
def random_full_write_and_inference(filename, num_test=1000) :
    ...
    with open(filename, "a") as file : 
        ...
        inference_log(0, 0, 0, 0, Array)
        print(Array[0][0][0], Array[1][0][0], Array[2][0][0], Array[3][0][0])
        inference_log(0, 1, 2, 3, Array)
        print(Array[0][0][0], Array[1][0][1], Array[2][0][2], Array[3][0][3])

        test_points = np.random.randint(0, 512, (num_test, 4))

        for i in range(num_test) :
            #inference_log(test_points[i][0], test_points[i][1], test_points[i][2], test_points[i][3], Array)
            ...
        
        # stoch mode
        file.write(f"write | 201C | {0:08X}\n")

        def inference_stoch( obs1, obs2, obs3, obs4, array) : 
            array = array.astype(np.uint16)
            file.write(f"write | 200C | {obs1:08X}\n")
            file.write(f"write | 2010 | {obs2:08X}\n")
            file.write(f"write | 2014 | {obs3:08X}\n")
            file.write(f"write | 2018 | {obs4:08X}\n")
            expected_data0 = array[0][obs1//8][obs1%8]/255 * array[1][obs2//8][obs2%8]/255 * array[2][obs3//8][obs3%8]/255 * array[3][obs4//8][obs4%8]
            expected_data1 = array[4][obs1//8][obs1%8]/255 * array[5][obs2//8][obs2%8]/255 * array[6][obs3//8][obs3%8]/255 * array[7][obs4//8][obs4%8]
            expected_data2 = array[8][obs1//8][obs1%8]/255 * array[9][obs2//8][obs2%8]/255 * array[10][obs3//8][obs3%8]/255 * array[11][obs4//8][obs4%8]
            expected_data3 = array[12][obs1//8][obs1%8]/255 * array[13][obs2//8][obs2%8]/255 * array[14][obs3//8][obs3%8]/255 * array[15][obs4//8][obs4%8]
            expected_array = np.array([expected_data0, expected_data1, expected_data2, expected_data3], dtype=np.uint32)
            expected_clip = np.clip(expected_array, 0, 255)
        ...
        inference_stoch(0, 0, 0, 0, Array)
        inference_stoch(0, 1, 2, 3, Array)
